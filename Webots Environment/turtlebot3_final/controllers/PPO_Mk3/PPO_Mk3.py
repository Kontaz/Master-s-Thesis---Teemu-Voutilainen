...
class Turtlebot3_RL(Supervisor, gym.Env):
    def __init__(self, max_episode_steps=1000):
        ...
        # boxes for different setups to validate the results
        self.__vbox0 = super().getFromDef('VALBOX0')
        self.__vbox1 = super().getFromDef('VALBOX1')
        self.__vbox2 = super().getFromDef('VALBOX2')
        self.__vbox3 = super().getFromDef('VALBOX3')
        
        self.__vbox0_loc = self.__vbox0.getField('translation')
        self.__vbox1_loc = self.__vbox1.getField('translation')
        self.__vbox2_loc = self.__vbox2.getField('translation')
        self.__vbox3_loc = self.__vbox3.getField('translation')
        
        self.__vbox0_rot = self.__vbox0.getField('rotation')
        self.__vbox1_rot = self.__vbox1.getField('rotation')
        self.__vbox2_rot = self.__vbox2.getField('rotation')
        self.__vbox3_rot = self.__vbox3.getField('rotation')
    ...
    def reset(self, seed=None):
        ...
        # setup options 0 for random, 5, 6, 7 for specific start location/directions
        setup = 0
        
        # placing robot and boxes
        if setup == 5: # Setup 5
            ...
            self.__vbox0_loc.setSFVec3f([0, 0, 0.1525])
            self.__vbox1_loc.setSFVec3f([-0.5, 0.25, -0.3])
            self.__vbox2_loc.setSFVec3f([0.5, 0.25, -0.3])
            self.__vbox3_loc.setSFVec3f([0.5, 0.75, -0.3])
            
            self.__vbox0_rot.setSFRotation([1, 0, 0, np.pi/2])
            self.__vbox1_rot.setSFRotation([0, 0, 1, 0])
            self.__vbox2_rot.setSFRotation([0, 0, 1, 0])
            self.__vbox3_rot.setSFRotation([0, 0, 1, 0])
        elif setup == 6: # Setup 6
            self.__robot_start_x = 0.8
            self.__robot_start_y = 0.8
            self.__start_loc.setSFVec3f([self.__robot_start_x,self.__robot_start_y,0])
            self.__start_dir.setSFRotation([0, 0, 1, -np.pi/4])
            
            self.__vbox0_loc.setSFVec3f([0.15, 0.75, 0.2225])
            self.__vbox1_loc.setSFVec3f([0.75, 0.15, 0.2225])
            self.__vbox2_loc.setSFVec3f([0.5, 0.25, -0.3])
            self.__vbox3_loc.setSFVec3f([0.5, 0.75, -0.3])
            
            self.__vbox0_rot.setSFRotation([0, 0, 1, np.pi/4])
            self.__vbox1_rot.setSFRotation([0, 0, 1, np.pi/4])
            self.__vbox2_rot.setSFRotation([0, 0, 1, 0])
            self.__vbox3_rot.setSFRotation([0, 0, 1, 0])
        elif setup == 7: # Setup 7
            self.__robot_start_x = -0.8
            self.__robot_start_y = -0.8
            self.__start_loc.setSFVec3f([self.__robot_start_x,self.__robot_start_y,0])
            self.__start_dir.setSFRotation([0, 0, 1, (np.pi/2 + np.pi/4)])
            
            self.__vbox0_loc.setSFVec3f([-0.76, 0, 0.2225])
            self.__vbox1_loc.setSFVec3f([-0.25, 0, 0.2225])
            self.__vbox2_loc.setSFVec3f([0.25, 0, 0.2225])
            self.__vbox3_loc.setSFVec3f([0.76, 0, 0.2225])
            
            self.__vbox0_rot.setSFRotation([0, 0, 1, 0])
            self.__vbox1_rot.setSFRotation([0, 0, 1, 0])
            self.__vbox2_rot.setSFRotation([0, 0, 1, 0])
            self.__vbox3_rot.setSFRotation([0, 0, 1, 0])
        else:
